...
class HistoryAdapter :RecyclerView.Adapter<HistoryAdapter.HistoryViewHolder>() {
    ...
    fun setData(data:List<MyDiary>){ //data가 변경될 경우 이런식으로 노티파이를 해줘야 새로그려준다.
        this.data = data
    ...
    override fun onBindViewHolder(holder: HistoryViewHolder, position: Int) {
        val diary= data[position] //레코드 데이터를 부른다.
        holder.textViewDate.text = diary.date //부른 데이터의 시간과 음식을 홀더 시간 음식에 저장한다.
        holder.textViewTitle.text = diary.title
    }
    ...
    class HistoryViewHolder(v: View): RecyclerView.ViewHolder(v){ //item_history에 기록된 view를 작성한다. 여기서 v는 컨스트럭트이다.
        val textViewDate: TextView = v.findViewById(R.id.textViewDate) //데이터항목을 ui에 덮어씌운다.
